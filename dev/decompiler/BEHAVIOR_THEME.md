...
- `OnPaint` (UCThemeLocal.cs:345) — draws each thumbnail centered in its 120-box + name 20px below, then a badge overlay layer, then panel bg `imageBk` on top, then knob if scrollable; key branches: `isLunbo`→draw carousel badges `P轮播1..6` (index = position in `lunBoArray`, else `P轮播选框`); `myMode<2`→draw close-X `P关闭按钮2` on items **index≥5 only** (defaults are protected); `myMode>=2`→close-X on **all** (user themes all deletable). `isOffsetY`→draw scrollbar knob at 702. [GOD paint].
