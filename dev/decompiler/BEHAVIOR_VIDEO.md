...
- `buttonTPJCH_Click` (UCBoFangQiKongZhi.cs:1301) — [COPY-PASTE] switches the video fit mode from width-fill to **height-fill** (letterbox left/right): for the active panel it sets the output canvas `wValSub`/`hValSub` to the panel's own dimensions, resets `xVal`/`yVal` to 0, pins `hVal` to the canvas height, derives `wVal` = `bitAngleW` * height / `bitAngleH`, re-centres horizontally by adding (canvasWidth - wVal)/2 to `xVal`, and clears `isTPJCW`. Every branch is the same six assignments with different constants. Key branches: `isTPJCW false -> return immediately` (already in height-fill mode, :1303-1306); playback is suspended first (`isStart` captured into a local and set false, :1307-1308) — the restore of that captured value lies past line 1451 and was outside the assigned range; `is240x240 -> 240 canvas` (:1309); `is320x320 -> 320` (:1320); `is360x360 -> 360` (:1331); `is480x480 -> 480` (:1342); `is176x320 and isFanZhuan -> 176×320 canvas, hVal 320`, `else -> 320×176 canvas, hVal 176` (:1353-1377); `is1600x720 and isFanZhuan -> 720×1600, hVal 1600`, `else -> 1600×720, hVal 720` (:1378-1402); `is800x480 and isFanZhuan -> 480×800, hVal 800`, `else -> 800×480, hVal 480` (:1403-1427); `is854x480 and isFanZhuan -> 480×854, hVal 854`, `else -> 854×480, hVal 480` (:1428-1452). The branch chain continues past line 1451 (the 960×540 case and any later ones) — outside the assigned range, nothing asserted about it.
...
- `buttonYulan_Click` (UCVideoCut.cs:1504) — starts the clip preview: kills any running preview ffmpeg (`KillFFmpegYulan`), wipes the frame directory (`ResetAllTempFile`), and shells `cmd.exe` (no window, stdin/stdout redirected) to `cd /d "<Application.StartupPath>"` then run ffmpeg with `-display_rotation <bitAngle> -ss <LongToTimer(startTimerVal)> -t <LongToTimer(stopTimerVal - startTimerVal)> -i "<myName>" -y -r <originalImageHz> -s <wVal>x<hVal> -f image2 "<allPicAddr>%04d.bmp"` (:1518-1530). The process handle is closed immediately, so ffmpeg runs detached and the frames appear asynchronously. Then latches `yuLanHz` = `originalImageHz`, sets `isYulan` and swaps the button art to `Resources.P0预览a`. Key branches: `isZhuanma true (a transcode is running) -> nothing happens`; `isYulan already true -> nothing happens` (:1506); otherwise the preview launches. Note the rotation is applied by ffmpeg via `-display_rotation`, not in managed code.
...
- `GetOneImage` (UCVideoCut.cs:1580) — grabs a single scrub-position thumbnail. Guards re-entry with `isGetOneImage`, deletes the previous `onePic` file, shells `cmd.exe` to `cd /d "<Application.StartupPath>"` then ffmpeg with `-display_rotation <bitAngle> -ss <timer> -i "<myName>" -y -s <wVal>x<hVal> -frames:v 1 -f image2 "<onePic>"` (:1604-1606) — input-side seek, exactly one frame, scaled to the fit size. It then polls for the output file in 1 ms sleeps. On success it rebuilds `imageSub` as a fresh `wValSub`×`hValSub` bitmap, blits the grabbed frame at (`xVal`,`yVal`), invalidates the control and clears the re-entry flag. Key branches: `isGetOneImage already true -> return, no grab`; `requested position + 1000 ms exceeds originalTimerLen -> position clamped to originalTimerLen - 1000 and the timer string regenerated` (:1599-1603); `file still absent after 100 polls (~100 ms) -> isGetOneImage cleared and return` — on this path the Process object is never closed or disposed (:1607-1617), unlike the success path (:1618-1619); `imageSub non-null -> disposed before replacement` (:1620-1624).
...
- `UCBoFangQiKongZhi` (UCBoFangQiKongZhi.cs:144) — constructor for the video-player control. Calls `InitializeComponent`, then clears the flat-button borders on `button1`, `buttonTPJCH` and `buttonTPJCW` (transparent ARGB 0,0,0,0). Resolves the working directory `boFangQiMuLu` = `Form1.AddDataFilePath("Player\\")` (EXTERNAL: Form1.cs) and the fixed source path `boFangQiFile` = that directory + `1.mp4`; records `boFangQiFileExist`. Initialises the progress-bar overlay: `bitmapJindu` = `Resources.P进度条`, drawn at x=10, y=10, with `bitmapJDW`=0 (grows with playback) and `bitmapJDH` = the resource's own height. Key branches: `1.mp4 already exists -> it is deleted before use` (:153-162), delete failure swallowed.
...
- `LongToTimer` (UCBoFangQiKongZhi.cs:191) — converts a millisecond count into the `HH:MM:SS.mmm` string ffmpeg is fed as `-ss` / `-t` (hours from val/60000/60, minutes and seconds mod 60, milliseconds mod 1000; hours/minutes/seconds zero-padded to 2 digits, milliseconds to 3). Exact inverse of `TimerToLong` (:180), which parses the same shape back out of ffmpeg's `Duration:` line and rounds to 2 decimal places before scaling by 1000. No branches.
