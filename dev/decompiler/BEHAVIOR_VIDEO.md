...
- `BitmapToByte(Bitmap Bit)` (UCVideoCut.cs:1179) — JPEG-encodes a bitmap to `byte[]` via `MemoryStream` + `ImageFormat.Jpeg`; key branches: retry-loop `while(flag)` — on exception `Thread.Sleep(100)`+`DoEvents()` and retry (never gives up). Note `GetBuffer()` returns the *padded* stream buffer, not `ToArray()`.
