...
- `ucComboBoxB(mode)` (FormLED.cs:1978) — memory-ratio picker (style4/LC1). `mode`: 1→`memoryRatio=1`, 2→`=2`, 3→`=4`; then `SetMyNameFile()`.
...
- `buttonCF_Click(mode)` (FormLED.cs:2417) — temp-unit selector: `myTempMode=mode`; resets both C/F button skins to `P点选框`, then `mode`: 1→highlight buttonC, 2→highlight buttonF (`P点选框A`).
