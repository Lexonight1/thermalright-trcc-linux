...
- `OnPaint` (FormScreenImage.cs:85) — chains to the base Form paint, then blits the form's own `BackgroundImage` at (0,0) at its native size, ignoring the form's `BackgroundImageLayout`. Because the background resource is `P0预览弹窗800X360` (800×360) while `ClientSize` is 820×420 (:143, :145), the artwork does not cover the client area — the remaining band shows the White `BackColor`. No branches.
...
- `InitializeComponent` (FormScreenImage.cs:108) — designer plumbing: a borderless, always-on-top, non-resizable 820×420 popup (`FormBorderStyle` 0 = None, `TopMost`, `MaximizeBox`/`MinimizeBox` off, `StartPosition` 1 = CenterScreen, `DoubleBuffered` on) carrying the `P0预览弹窗800X360` background and one 40×40 transparent flat close button `buttonPower` at (776, 2) using `Resources.Alogout默认`; wires the button's Click/Paint and the form's MouseDown/MouseMove/MouseUp (the drag-to-move set, since the form has no title bar).
