...
One widget for both Qt skins (``ui/gui`` and ``ui/qtgui`` each carried a
byte-for-byte equivalent copy) — same geometry constants, same magnifier,
same Windows ``FormGetColor`` 12x12 capture area.
"""
...
class EyedropperOverlay(BaseScreenOverlay):
    ...
    def mousePressEvent(self, event) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            log.info("EyedropperOverlay.mousePressEvent: button=%s cancel",
                     event.button())
            self._cancel()
