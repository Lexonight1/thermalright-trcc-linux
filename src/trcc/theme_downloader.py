...
class ThemeDownloader:
    """Download and manage TRCC theme packs."""
    ...
    def get_installed_packs() -> Dict[str, dict]:
        ...
        themes_dir = ThemeDownloader.get_user_themes_dir()
    ...
    def list_available() -> None:
        ...
        installed = ThemeDownloader.get_installed_packs()
    ...
    def show_info(pack_name: str) -> None:
        ...
        if pack_name in installed:
            ...
            print(f"  Location:  {ThemeDownloader.get_user_themes_dir() / info['resolution'].replace('x', '')}")
        else:
            print("\nInstalled:   No")
    ...
    def download_pack(pack_name: str, force: bool = False) -> int:
        ...
        # Prepare paths
        cache_dir = ThemeDownloader.get_cache_dir()
        archive_name = f"{pack_name}-{info['version']}.tar.gz"
        archive_path = cache_dir / archive_name

        # Download (use cache if exists and valid)
        if archive_path.exists() and ThemeDownloader.verify_checksum(archive_path, info.get("sha256")):
            print(f"Using cached download: {archive_path}")
        else:
            # Try primary URL
            url = info["url"]
            if not ThemeDownloader.download_with_progress(url, archive_path, f"Downloading {pack_name}"):
                # Try mirror if available
                if pack_name in MIRROR_URLS:
                    print("Trying mirror...")
                    if not ThemeDownloader.download_with_progress(MIRROR_URLS[pack_name], archive_path, "Downloading"):
                        return 1
                else:
                    return 1

            # Verify checksum
            if not ThemeDownloader.verify_checksum(archive_path, info.get("sha256")):
                archive_path.unlink()
                return 1
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not ThemeDownloader.extract_archive(archive_path, temp_path):
                return 1
            ...
            for item in temp_path.iterdir():
                if item.is_dir():
                    ...
                    theme_count = sum(1 for d in item.iterdir()
                                    if d.is_dir() and ThemeDownloader._is_theme_subdir(d))
                    if theme_count > 0:
            ...
            dest_dir = ThemeDownloader.get_user_themes_dir() / resolution
...
get_user_themes_dir = ThemeDownloader.get_user_themes_dir
get_cache_dir = ThemeDownloader.get_cache_dir
get_installed_packs = ThemeDownloader.get_installed_packs
list_available = ThemeDownloader.list_available
show_info = ThemeDownloader.show_info
download_with_progress = ThemeDownloader.download_with_progress
verify_checksum = ThemeDownloader.verify_checksum
extract_archive = ThemeDownloader.extract_archive
download_pack = ThemeDownloader.download_pack
remove_pack = ThemeDownloader.remove_pack
create_local_pack = ThemeDownloader.create_local_pack
...
if __name__ == "__main__":
    ...
    if args.command == "list":
        ThemeDownloader.list_available()
    elif args.command == "info":
        if not args.pack:
            print("Usage: theme_downloader.py info <pack-name>")
            sys.exit(1)
        ThemeDownloader.show_info(args.pack)
    elif args.command == "download":
        if not args.pack:
            print("Usage: theme_downloader.py download <pack-name>")
            sys.exit(1)
        sys.exit(ThemeDownloader.download_pack(args.pack, force=args.force))
    elif args.command == "remove":
        if not args.pack:
            print("Usage: theme_downloader.py remove <pack-name>")
            sys.exit(1)
        sys.exit(ThemeDownloader.remove_pack(args.pack))
        ...
        if not args.pack or not args.source or not args.resolution:
            ...
            sys.exit(1)
        sys.exit(ThemeDownloader.create_local_pack(args.source, args.pack, args.resolution))
